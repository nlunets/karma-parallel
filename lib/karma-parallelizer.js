...
function initKarmaParallelizer(root, karma, shardIndexInfoMap) {
  ...
  var count = 0;
  var tests = [];
  karma.config.ui5.shardIndex = shardIndexInfo.shardIndex;
  if(shardIndex > executors && executors === 1) {
    // Tests are relaunched
   shardIndex = shardIndex % executors;
  }

  if(Array.isArray(karma.config.ui5.tests[0])) {
    karma.config.ui5.tests = karma.config.ui5.tests[shardIndex].concat();
  }
  else {
    karma.config.ui5.tests.forEach((testName) => {
      if(count++ % executors === shardIndex) {
        tests.push(testName);
      }
    })
    karma.config.ui5.tests = tests;
  }
